...
describe('HttpServiceMemory', () => {
  ...
  it('should return a 404 rejected promise if a route is not set', async () => {
    ...
    const request = new HttpRequestBuilder().get().url('https://local.gaf.energy').build();

    // Act.
    const actual = service.request(request);

    // Assert.
    await expect(actual).rejects.toEqual(expect.objectContaining({ status: 404 }));
  });
  ...
  it('should return a 404 rejected promise if a route is not set with an associated method', async () => {
    ...
    const service = createService();
    const request = new HttpRequestBuilder().get().url('https://local.gaf.energy').build();
    const response = () => new HttpResponseBuilder().status(200).data('OK').build();
  ...
  it('should return a rejected promise if the status code is a server error', async () => {
    // Arrange.
    const service = createService();
    const response = new HttpResponseBuilder().data({ message: 'I messed up' }).status(500).build();
    const request = new HttpRequestBuilder().url('https://gaf.energy').delete().build();
  ...
  it('should return the mapped url to result', async () => {
    ...
    const service = createService();
    const request = new HttpRequestBuilder().get().url('https://local.gaf.energy').build();
    const response = new HttpResponseBuilder().status(200).data({ field: 'looks-good' }).build();
